...
export default function AdminSidebar({ isOpen, onClose, isCollapsed, onToggleCollapse }) {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside
                className={`fixed top-0 left-0 h-full z-30 bg-slate-900 border-r border-slate-800 flex flex-col transition-all duration-300 ease-in-out
                    ${isCollapsed ? 'w-20' : 'w-60'}
                    ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
            >
                {/* Logo */}
                <div className={`flex items-center h-16 px-4 border-b border-slate-800 shrink-0 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
                    {!isCollapsed && (
                        <div>
                            <span className="text-white font-bold text-base">AgriSupport</span>
                            <span className="block text-emerald-400 text-xs font-semibold uppercase tracking-widest">Admin</span>
                        </div>
                    )}
                    <button
                ...
                <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
                    {menuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <span className={`shrink-0 ${active ? 'text-emerald-400' : ''}`}>
                                    {item.icon}
                                </span>
