...
function ContactCard({ icon, title, value, href }) {
    const content = (
        <div className="bg-white border border-[#e2e8f0] rounded-2xl p-5 flex items-start gap-4 hover:border-[#007a55] hover:shadow-md transition-all group">
            <div className="w-11 h-11 bg-[#f0f9f4] rounded-xl flex items-center justify-center shrink-0 group-hover:bg-[#007a55] transition-colors">
                <span className="text-[#007a55] group-hover:text-white transition-colors">{icon}</span>
            </div>
            <div>
                <p className="text-[12px] font-semibold text-[#90a1b9] uppercase tracking-wide mb-1">{title}</p>
                <p className="text-[14px] font-bold text-[#0f172b]">{value}</p>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen font-['Inter',sans-serif] flex flex-col">
            ...
            <main className="flex-1 max-w-[800px] w-full mx-auto px-6 py-12">
                <div className="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl p-8 md:p-12">
                    <h1 className="text-[30px] font-extrabold text-[#0f172b] mb-2">Kontak Kami</h1>
                    <p className="text-[15px] text-[#62748e] mb-10">
                    ...
                    <div className="border border-[#e2e8f0] rounded-2xl p-6">
                        <h2 className="text-[18px] font-bold text-[#0f172b] mb-5">Tim Pengembang — Kelompok E</h2>
                        <div className="space-y-3">
                            {[
                                ...
                                <div key={member.name} className="flex items-center gap-4 py-3 border-b border-[#f1f5f9] last:border-0">
                                    <div className="w-9 h-9 bg-[#f0f9f4] rounded-full flex items-center justify-center shrink-0">
                                        <span className="text-[13px] font-bold text-[#007a55]">{member.name.charAt(0)}</span>
                                    </div>
                                    <div>
                                        <p className="text-[14px] font-bold text-[#0f172b]">{member.name}</p>
                                        <p className="text-[12px] text-[#62748e]">{member.role}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
